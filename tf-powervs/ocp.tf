...
module "nodes" {
  ...
  system_type                     = var.system_type
  network_name                    = var.network_name
  bastion_ip                      = lookup(var.bastion, "count", 1) > 1 ? module.bastion.bastion_vip : module.bastion.bastion_ip[0]
  ...
  worker                          = var.worker
  volume_shareable                = var.volume_shareable
  bastion_external_vip            = module.bastion.bastion_external_vip
...
module "config_ocp" {
  ...
  depends_on = [module.setup_bastion, module.nodes]
  ...
  public_cidr                    = module.bastion.public_cidr
  bastion_count                  = lookup(var.bastion, "count", 1)
  bastion_vip                    = module.bastion.bastion_vip
  ...
  ssh_agent                      = var.ssh_agent
  connection_timeout             = var.connection_timeout
  bastion_internal_vip           = module.bastion.bastion_internal_vip
  bastion_external_vip           = module.bastion.bastion_external_vip
  bastion_public_ip              = module.bastion.bastion_public_ip
...
module "ibmcloud" {
  count  = var.use_ibm_cloud_services ? 1 : 0
  source = "./modules/4_ibmcloud"
  providers = {
    ibm = ibm.classic
  }
  depends_on = [module.bastion, module.module.nodes]
...
module "install" {
  depends_on = [module.config_ocp, module.ibmcloud, module.restart]
